...
import { IApiMeta, IApiResponse, ITag } from '@/interfaces';
import { deleteTagMutation } from '@/mutations';
import { tagService } from '@/services/tagService';
import { displayDate } from '@/utils/displayDateUtil';

import { CreateAndEditTagModal } from '../CreateAndEditTagModal';

type TagsTableProps = {
  initalTagsResponse: IApiResponse<ITag[]>;
};

const { Paragraph, Text } = Typography;

export const TagsTable: FC<TagsTableProps> = ({ initalTagsResponse }) => {
  ...
  const { searchParams, handleChangePagination, handleChangeFilter } =
    usePagination({
      meta: initalTagsResponse?.meta as IApiMeta,
    });
  ...
  const {
    ...
    refetch,
    isRefetching,
  } = useQueryFactory<ITag[]>({
    queryKey: [QueryKeys.TAGS, searchParams],
    initialData: initalTagsResponse,
    queryFn: () => tagService.all(`${searchParams.toString()}`),
  });
  const loading = isRefetching || isLoading;
  const tags = tagsResponse?.result || [];

  // CREATE, DELETE AND EDIT TAG FUNCTIONS AND STATES
  const [isCreateAndEditTagModalOpen, setCreateAndEditTagModalOpen] =
    useState(false);
  const [currentTagUpdate, setCurrentTagUpdate] = useState<ITag | null>(null);
